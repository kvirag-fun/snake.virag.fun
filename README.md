...
| `swipe.spec.js` | The touch swipe-to-direction resolution itself - blocked reversals, chaining multiple turns in one continuous drag, redundant same-direction crossings not spamming the queue, and a diagonal drag not spuriously firing the axis it never meant to turn on - driven through raw CDP touch events (`Input.dispatchTouchEvent`), not just poking `inputQueue` |
